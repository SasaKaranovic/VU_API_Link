...
!define APPNAME "API Link"
!define COMPANYNAME "KaranovicResearch"
!define DESCRIPTION "API Link application"
# These three must be integers
!define VERSIONMAJOR {{API_LINK_VERSION_MAJOR}}
!define VERSIONMINOR {{API_LINK_VERSION_MINOR}}
!define VERSIONBUILD {{API_LINK_VERSION_BUILD}}
...
# Executable
!define MAINEXE VU-API-Link.exe
...
;General
  ...
  InstallDir "$PROGRAMFILES\KaranovicResearch\VU-API-Link"

  ;Get installation folder from registry if available
  InstallDirRegKey HKCU "Software\VU-API-Link" ""
...
Section "VU-API-Link Server" VUDSERVER
  ...
  ;WriteRegStr HKCU "Software\VU-API-Link\install_path" "" $INSTDIR
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Run" "VU-API-Link" '"$InstDir\VU-API-Link.exe"'

  ; Run the VU server
  ExecShell "" "$InstDir\VU-API-Link.exe"
...
Section "Uninstall"
  ...
  Delete "$smprograms\VU-API-Link\"
  RMDir /r "$INSTDIR"

  ; Remove windows start
  DeleteRegValue HKLM "Software\Microsoft\Windows\CurrentVersion\Run" "VU-API-Link"
